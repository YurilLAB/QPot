...
myANSIBLE_QPOT_PLAYBOOK="installer/install/deploy.yml"
myADJECTIVE=$(shuf -n1 installer/install/a.txt)
myNOUN=$(shuf -n1 installer/install/n.txt)
myENV_FILE="$HOME/qpotce/.env"

myDEPLOY=$(cat << "EOF"

  ____        _           ____             _
 / ___|  ___ | |_   _ __ |  _ \  ___ _ __ | | ___  _   _
 \___ \ / _ \| __| |  _ \| | | |/ _ \  _ \| |/ _ \| | | |
...
if ! grep -q 'QPOT_TYPE=HIVE' "$HOME/qpotce/.env";
  then
...
echo "${myDEPLOY}"
echo
echo "# This script will prepare a QPot SENSOR installation to transmit logs into this HIVE."
echo

# Ask if a QPot SENSOR was installed
read -p "# Was a QPot SENSOR installed? (y/n): " mySENSOR_INSTALLED
if [[ ${mySENSOR_INSTALLED} != "y" ]]; 
    then
      echo "# A QPot SENSOR must be installed to continue."
      exit 1
fi

# Ask for the remote user
read -p "# Enter the remote username QPot SENSOR was installed with: " mySSHUSER
if [[ ${mySSHUSER} == "" ]]; 
    then
...
while true; do
  read -p "# Enter the IP/domain name of this HIVE: " myQPOT_HIVE_IP
  if [[ ${myQPOT_HIVE_IP} =~ ^([a-zA-Z0-9]+(\.[a-zA-Z0-9]+)*\.[a-zA-Z]{2,})|(([0-9]{1,3}\.){3}[0-9]{1,3})$ ]]; 
    then
      break
    else
      echo "# Invalid IP/domain. Please enter a valid IP or domain name."
...
# Create myQPOT_HIVE_USER, since this is for Logstash on the SENSOR, it needs to directly base64 encoded
myQPOT_HIVE_USER=$(echo -n "${myLS_WEB_USER}:${myLS_WEB_PW}" | base64 -w0)
...
echo "# New SENSOR credentials base64 encoded: ${myQPOT_HIVE_USER}"
echo
...
export myQPOT_HIVE_USER
export myQPOT_HIVE_IP

ANSIBLE_LOG_PATH=${HOME}/qpotce/data/deploy_sensor.log ansible-playbook ${myANSIBLE_QPOT_PLAYBOOK} -i ${mySENSOR_IP}, -c ssh -u ${mySSHUSER} --ask-become-pass -e "ansible_port=${myANSIBLE_PORT}"
...
if [ "$?" == 0 ];
  then
	...
	: > "${HOME}"/qpotce/data/nginx/conf/lswebpasswd
	  ...
	  do
	    if [[ -n $i ]]; 
	      then
	        ...
	        echo >> ${HOME}/qpotce/data/nginx/conf/lswebpasswd
	      fi
	done
fi

unset myQPOT_HIVE_USER
unset myQPOT_HIVE_IP
